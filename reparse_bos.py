#!/usr/bin/env python3
"""
Re-parse BOS configs with the updated parser
Fixes "RNAV 27, DEP 33L" pattern recognition
"""
...
def reparse_bos_configs():
    """Re-parse BOS configs with updated parser"""
    ...
    # Find all BOS configs (especially those with RNAV pattern)
    cursor.execute("""
    ...
    print(f"Found {len(configs)} BOS configs to re-parse")
    ...
    for config in configs:
        ...
        if (new_arriving != old_arriving or
            ...
            fixed_count += 1
            print(f"Re-parsed BOS config {config['id']}:")
    ...
    print(f"\n=== Summary ===")
    print(f"Total BOS configs re-parsed: {fixed_count}")

    cursor.close()
    conn.close()

if __name__ == "__main__":
    print("Re-parsing BOS configs with updated parser...")
    reparse_bos_configs()
    print("\nDone!")
